...
struct DepositDef
{
    ...
    float richnessOffset;  // added to procedural noise before the fixed threshold
...
static void LoadDepositRegistry()
{
    ...
    for (char* line = strtok_s(buf, "\n", &ctx); line; line = strtok_s(NULL, "\n", &ctx))
    {
        ...
        *eq = 0;
        ...
        else if (KeyIs(line, "richness_offset"))
        {
            char* end = NULL;
            double parsed = strtod(val, &end);
            if (!end || *end || !isfinite(parsed) || parsed < -0.25 || parsed > 0.25)
            {
                d->richnessOffset = 0.0f;
                Logf("deposits  \"%s\": invalid richness_offset \"%s\"; using 0.00",
                     d->name, val);
            }
            else d->richnessOffset = (float)parsed;
        }
...
// ---------------------------------------------------------------- generated deposit population
//
// Every declared extra deposit receives its own distribution, regardless of
// whether the base terrain was hand-authored or procedural. A small manifest
// saved beside the world records the per-game seed and exactly which catalogued
// type owns each initialized channel. This lets a later mod fill one new channel
// without touching the other three bytes in the same DDS.

#define DDS_BYTES (128u + (unsigned)MAP_EDGE * MAP_EDGE * 4u)
#define DEPOSIT_MANIFEST "soviet_mod_loader_deposits.ini"
#define MANIFEST_FORMAT 1

struct WorldDepositState
{
    ...
    int map[128];
    int component[128];
    bool saveEligible[MAX_EXTRA_MAPS];
};

static WorldDepositState g_worldState;
static bool g_smlPatchReady;
static bool g_smlMapsReady;

static unsigned __int64 SmlHashBytes(unsigned __int64 h, const void* data, size_t n)
{
...
static unsigned __int64 SmlMix64(unsigned __int64 x)
{
...
static bool HasParentSegment(const char* path)
{
    const char* p = path;
    while (p && *p)
    {
        ...
        const char* begin = p;
        while (*p && *p != '/' && *p != '\\') p++;
        if (p - begin == 2 && begin[0] == '.' && begin[1] == '.') return true;
    }
    return false;
}

static bool PathInside(const char* root, const char* candidate)
{
    size_t len = strlen(root);
    return _strnicmp(root, candidate, len) == 0 &&
           (candidate[len] == 0 || candidate[len] == '\\' || candidate[len] == '/');
}

static bool EnsureDirectoryTree(const char* path)
{
    char work[MAX_PATH];
    strncpy_s(work, sizeof(work), path, _TRUNCATE);
    for (char* p = work + 3; *p; p++)
    {
        if (*p != '\\' && *p != '/') continue;
        char saved = *p; *p = 0;
        if (!CreateDirectoryA(work, NULL) && GetLastError() != ERROR_ALREADY_EXISTS)
            { *p = saved; return false; }
        *p = saved;
    }
    return CreateDirectoryA(work, NULL) || GetLastError() == ERROR_ALREADY_EXISTS;
}

static bool ResolveUnderMediaRoot(const char* mediaRootInput, const char* folder,
                                  char* out, size_t n, bool create)
{
    if (!folder || !*folder || HasParentSegment(folder))
    {
        SetLastError(ERROR_INVALID_NAME);
        return false;
    }

    char mediaRoot[MAX_PATH];
    if (!GetFullPathNameA(mediaRootInput, sizeof(mediaRoot), mediaRoot, NULL)) return false;

    char raw[MAX_PATH];
    bool absolute = (isalpha((unsigned char)folder[0]) && folder[1] == ':') ||
                    (folder[0] == '\\' && folder[1] == '\\');
    if (absolute)
        strncpy_s(raw, sizeof(raw), folder, _TRUNCATE);
    else
    {
        const char* rel = folder;
        if (_strnicmp(rel, "media_soviet/", 13) == 0 ||
            _strnicmp(rel, "media_soviet\\", 13) == 0) rel += 13;
        if (!*rel || strchr(rel, ':')) { SetLastError(ERROR_INVALID_NAME); return false; }
        _snprintf_s(raw, sizeof(raw), _TRUNCATE, "%s\\%s", mediaRoot, rel);
    }

    char full[MAX_PATH];
    if (!GetFullPathNameA(raw, sizeof(full), full, NULL) || !PathInside(mediaRoot, full))
    {
        SetLastError(ERROR_ACCESS_DENIED);
        return false;
    }
    for (char* p = full; *p; p++) if (*p == '/') *p = '\\';
    if (create && !EnsureDirectoryTree(full)) return false;
    strncpy_s(out, n, full, _TRUNCATE);
    return true;
}
...
extern "C" int SmlDepositsResolveWorldPathForTest(const char* mediaRoot, const char* folder,
                                                    char* out, size_t n)
{
    return ResolveUnderMediaRoot(mediaRoot, folder, out, n, false) ? 1 : 0;
}

static bool ResolveWorldDirectory(const char* folder, char* out, size_t n, bool create)
{
    char exePath[MAX_PATH], gameRoot[MAX_PATH], mediaRootRaw[MAX_PATH];
    if (H && !H->exeModule && H->baseDir)
    {
        strncpy_s(gameRoot, sizeof(gameRoot), H->baseDir, _TRUNCATE);
        char* slash = strrchr(gameRoot, '\\');
        char* alt = strrchr(gameRoot, '/');
        if (!slash || (alt && alt > slash)) slash = alt;
        if (!slash) { SetLastError(ERROR_INVALID_NAME); return false; }
        *slash = 0;
    }
    else
    {
        HMODULE module = H && H->exeModule ? (HMODULE)H->exeModule : GetModuleHandleA(NULL);
        DWORD got = GetModuleFileNameA(module, exePath, sizeof(exePath));
        if (!got || got >= sizeof(exePath)) return false;
        strncpy_s(gameRoot, sizeof(gameRoot), exePath, _TRUNCATE);
        char* slash = strrchr(gameRoot, '\\');
        char* alt = strrchr(gameRoot, '/');
        if (!slash || (alt && alt > slash)) slash = alt;
        if (!slash) { SetLastError(ERROR_INVALID_NAME); return false; }
    ...
    _snprintf_s(mediaRootRaw, sizeof(mediaRootRaw), _TRUNCATE, "%s\\media_soviet", gameRoot);
    return ResolveUnderMediaRoot(mediaRootRaw, folder, out, n, create);
}

static bool ResolveWorldFile(const char* folder, const char* name, char* out, size_t n)
{
    char directory[MAX_PATH];
    if (!ResolveWorldDirectory(folder, directory, sizeof(directory), false)) return false;
    _snprintf_s(out, n, _TRUNCATE, "%s\\%s", directory, name);
    DWORD attr = GetFileAttributesA(out);
    return attr != INVALID_FILE_ATTRIBUTES && !(attr & FILE_ATTRIBUTE_DIRECTORY);
}

static unsigned __int64 NewWorldSeed(const char* folder)
{
    unsigned __int64 seed = 0;
    HMODULE advapi = GetModuleHandleA("advapi32.dll");
    typedef BOOLEAN (WINAPI* t_RtlGenRandom)(PVOID, ULONG);
    t_RtlGenRandom random = advapi ? (t_RtlGenRandom)GetProcAddress(advapi, "SystemFunction036") : NULL;
    if (random && random(&seed, sizeof(seed)) && seed) return seed;

    LARGE_INTEGER qpc; QueryPerformanceCounter(&qpc);
    FILETIME ft; GetSystemTimeAsFileTime(&ft);
    seed = ((unsigned __int64)ft.dwHighDateTime << 32) | ft.dwLowDateTime;
    seed ^= (unsigned __int64)qpc.QuadPart;
    seed ^= (unsigned __int64)GetCurrentProcessId() << 32;
    seed ^= (unsigned __int64)(ULONG_PTR)&g_worldState;
    if (folder) seed = SmlHashBytes(seed, folder, strlen(folder));
    return SmlMix64(seed ? seed : 1);
}

static bool ReadValidDds(const char* path, BYTE** out)
{
    *out = NULL;
    HANDLE h = CreateFileA(path, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                           NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (h == INVALID_HANDLE_VALUE) return false;
    LARGE_INTEGER size; bool ok = GetFileSizeEx(h, &size) != 0 && size.QuadPart == DDS_BYTES;
    BYTE* data = ok ? (BYTE*)malloc(DDS_BYTES) : NULL;
    DWORD got = 0;
    if (!data || !ReadFile(h, data, DDS_BYTES, &got, NULL) || got != DDS_BYTES) ok = false;
    CloseHandle(h);
    if (ok)
    {
        DWORD height = 0, width = 0;
        memcpy(&height, data + 12, 4); memcpy(&width, data + 16, 4);
        ok = memcmp(data, "DDS ", 4) == 0 && width == MAP_EDGE && height == MAP_EDGE;
    }
...
static bool LoadWorldManifest(const char* folder, WorldDepositState* state)
{
    memset(state, 0, sizeof(*state));
    for (int i = 0; i < 128; i++) { state->map[i] = -1; state->component[i] = -1; }

    char path[MAX_PATH];
    if (!ResolveWorldFile(folder, DEPOSIT_MANIFEST, path, sizeof(path)))
    {
        state->seed = NewWorldSeed(folder);
        Logf("maps     no deposit manifest for \"%s\"; new seed %016llx", folder,
    ...
    {
        state->seed = NewWorldSeed(folder);
        Logf("maps     manifest %s unreadable; preserving DDS files, new seed %016llx", path,
             (unsigned long long)state->seed);
        return false;
    }
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        if (*s == '[' && len > 2 && s[len - 1] == ']')
        {
            s[len - 1] = 0; strncpy_s(section, sizeof(section), s + 1, _TRUNCATE); continue;
        }
        char* eq = strchr(s, '=');
        if (!eq) { syntaxOk = false; continue; }
        *eq++ = 0; char* key = TrimLine(s); char* value = TrimLine(eq);
        if (_stricmp(section, "world") == 0 && _stricmp(key, "format") == 0)
            format = atoi(value);
        else if (_stricmp(section, "world") == 0 && _stricmp(key, "seed") == 0)
            state->seed = _strtoui64(value, NULL, 16);
        else if (_stricmp(section, "deposits") == 0)
        {
            int type = atoi(key), map = -1, component = -1;
            char* last = strrchr(value, '|');
            if (!last) { syntaxOk = false; continue; }
            *last++ = 0; component = atoi(last);
            last = strrchr(value, '|');
            if (!last) { syntaxOk = false; continue; }
            *last++ = 0; map = atoi(last);
            if (type < 10 || type > 127 || map < DEP_MAP_EXTRA || map >= MAX_MAPS ||
                component < 0 || component > 3)
                { syntaxOk = false; continue; }
            state->initialized[type] = true;
            state->map[type] = map;
            state->component[type] = component;
            entries++;
        }
    }
    fclose(f);
    if (!syntaxOk || format != MANIFEST_FORMAT || !state->seed)
    {
        memset(state->initialized, 0, sizeof(state->initialized));
        for (int i = 0; i < 128; i++) { state->map[i] = -1; state->component[i] = -1; }
        state->seed = NewWorldSeed(folder);
        Logf("maps     manifest %s invalid; preserving DDS files, new seed %016llx", path,
             (unsigned long long)state->seed);
        return false;
    }
    state->manifestValid = true;
    Logf("maps     manifest %s loaded: seed %016llx, %d deposit(s)", path,
         (unsigned long long)state->seed, entries);
    return true;
}

static float GridNoise(unsigned __int64 seed, int x, int y)
{
    unsigned __int64 v = seed ^ ((unsigned __int64)(unsigned)x * 0x9e3779b185ebca87ull)
                                  ^ ((unsigned __int64)(unsigned)y * 0xc2b2ae3d27d4eb4full);
    return (float)(SmlMix64(v) >> 40) * (1.0f / 16777215.0f);
}

static float Smooth(float t) { return t * t * (3.0f - 2.0f * t); }

static float ValueNoise(unsigned __int64 seed, int x, int y, int scale)
{
    int x0 = x / scale, y0 = y / scale;
    float fx = Smooth((float)(x % scale) / (float)scale);
    float fy = Smooth((float)(y % scale) / (float)scale);
    float a = GridNoise(seed, x0, y0), b = GridNoise(seed, x0 + 1, y0);
    float c = GridNoise(seed, x0, y0 + 1), d = GridNoise(seed, x0 + 1, y0 + 1);
    float top = a + (b - a) * fx, bottom = c + (d - c) * fx;
    return top + (bottom - top) * fy;
}

static BYTE ProceduralRichness(unsigned __int64 seed, int x, int y, float offset)
{
    float v = ValueNoise(seed, x, y, 128) * 0.50f +
              ValueNoise(seed ^ 0x51ed270b7d3a4c95ull, x, y, 64) * 0.27f +
              ValueNoise(seed ^ 0x94d049bb133111ebull, x, y, 32) * 0.15f +
              ValueNoise(seed ^ 0xbf58476d1ce4e5b9ull, x, y, 16) * 0.08f;
    const float threshold = 0.61f;
    v += offset;
    if (v < 0.0f) v = 0.0f;
    if (v > 1.0f) v = 1.0f;
    if (v <= threshold) return 0;
    float richness = (v - threshold) / (1.0f - threshold);
    richness = Smooth(richness);
    int byte = (int)(richness * 255.0f + 0.5f);
    return (BYTE)(byte < 0 ? 0 : byte > 255 ? 255 : byte);
}
...
extern "C" unsigned SmlDepositsNoiseChecksumForTest(unsigned __int64 seed)
{
    ...
    for (int y = 0; y < 64; y++) for (int x = 0; x < 64; x++)
        { hash ^= ProceduralRichness(seed, x * 16, y * 16, 0.0f); hash *= 16777619u; }
    return hash;
}
                                                               ...
                                                               float offset)
{
    unsigned hash = 2166136261u;
    for (int y = 0; y < 64; y++) for (int x = 0; x < 64; x++)
        { hash ^= ProceduralRichness(seed, x * 16, y * 16, offset); hash *= 16777619u; }
    return hash;
}
...
{
    unsigned __int64 total = 0; unsigned count = 0;
    for (int y = 0; y < 64; y++) for (int x = 0; x < 64; x++)
    {
        BYTE value = ProceduralRichness(seed, x * 16, y * 16, offset);
        total += value; if (value) count++;
    }
    if (sum) *sum = total;
    if (nonzero) *nonzero = count;
}

extern "C" int SmlDepositsShouldGenerateForTest(int manifestValid, int ddsExists,
                                                  int initialized)
{
    return (!ddsExists || (manifestValid && !initialized)) ? 1 : 0;
}
...
static unsigned __int64 DepositSeed(const WorldDepositState* state, const DepositDef* d)
{
    unsigned __int64 seed = SmlHashBytes(state->seed, &d->type, sizeof(d->type));
    seed = SmlHashBytes(seed, d->name, strlen(d->name));
    return SmlMix64(seed);
}

static void FillGeneratedChannel(BYTE* data, int component, unsigned __int64 seed, float offset)
{
    for (int y = 0; y < MAP_EDGE; y++) for (int x = 0; x < MAP_EDGE; x++)
    {
        size_t pixel = (size_t)y * MAP_EDGE + x;
        data[128 + pixel * 4 + component] = ProceduralRichness(seed, x, y, offset);
    }
}
...
extern "C" int SmlDepositsChannelIsolationForTest(unsigned __int64 seed, int component)
{
    ...
    BYTE* data = (BYTE*)malloc(DDS_BYTES);
    if (!data) return 0;
    memset(data, 0x5a, DDS_BYTES);
    FillGeneratedChannel(data, component, seed, 0.0f);
    bool changed = false, isolated = true;
    for (size_t pixel = 0; pixel < (size_t)MAP_EDGE * MAP_EDGE; pixel++)
        for (int c = 0; c < 4; c++)
            if (c == component) changed = changed || data[128 + pixel * 4 + c] != 0x5a;
            else isolated = isolated && data[128 + pixel * 4 + c] == 0x5a;
    free(data);
    return changed && isolated ? 1 : 0;
}

static const char* BuildGeneratedMap(const char* folder, WorldDepositState* state,
                                     int extraIndex, bool ownExists,
                                     bool pending[128], bool* invalidExisting)
{
    static char rel[MAX_PATH];
    *invalidExisting = false;
    memset(pending, 0, sizeof(bool) * 128);
    BYTE* data = NULL;
    if (ownExists)
    {
        char physical[MAX_PATH], name[40];
        _snprintf_s(name, sizeof(name), _TRUNCATE, "resourcemap%d.dds",
                    DEP_MAP_EXTRA + extraIndex + 1);
        if (!ResolveWorldFile(folder, name, physical, sizeof(physical)) ||
            !ReadValidDds(physical, &data))
        {
            *invalidExisting = true;
            Logf("maps     resourcemap%d exists but is unreadable or invalid; left untouched",
                 DEP_MAP_EXTRA + extraIndex + 1);
            return NULL;
        }
    }
    ...
    {
        data = (BYTE*)malloc(DDS_BYTES);
        if (!data) return NULL;
        memcpy(data, kBlankDdsHeader, sizeof(kBlankDdsHeader));
        memset(data + 128, 0, DDS_BYTES - 128);
    }
    ...
    {
        DepositDef* d = &g_dep[i];
        if (d->map != DEP_MAP_EXTRA + extraIndex || d->component < 0 || d->component > 3) continue;
        bool known = ownExists && state->manifestValid && state->initialized[d->type] &&
                     state->map[d->type] == d->map &&
                     state->component[d->type] == d->component;
        // A valid legacy DDS without a manifest is opaque. Preserve every
        // currently associated channel and bootstrap it into the new manifest.
        if (ownExists && !state->manifestValid) known = true;
        if (known)
        {
            if (ownExists && !state->manifestValid) pending[d->type] = true;
            preserved++;
            continue;
        }

        unsigned __int64 seed = DepositSeed(state, d);
        FillGeneratedChannel(data, d->component, seed, d->richnessOffset);
        pending[d->type] = true;
        Logf("maps     generated %-20s -> resourcemap%d component %d, fractal seed %016llx, "
             "richness_offset %+.3f, threshold 0.610",
             d->name, DEP_MAP_EXTRA + extraIndex + 1, d->component,
             (unsigned long long)seed, d->richnessOffset);
        generated++;
    }
    if (!generated)
    {
        free(data);
        Logf("maps     resourcemap%d: %d channel(s) preserved, no generation needed",
             DEP_MAP_EXTRA + extraIndex + 1, preserved);
        return ownExists ? "" : NULL;
    }

    char fallbackRoot[MAX_PATH], media[MAX_PATH], tesmio[MAX_PATH], proc[MAX_PATH], world[MAX_PATH];
    const char* vfsRoot = g_vfsRoot;
    if (!vfsRoot || !vfsRoot[0])
    {
        _snprintf_s(fallbackRoot, sizeof(fallbackRoot), _TRUNCATE, "%s\\vfs", g_baseDir);
        vfsRoot = fallbackRoot;
    }
    _snprintf_s(media, sizeof(media), _TRUNCATE, "%s\\media_soviet", vfsRoot);
    _snprintf_s(tesmio, sizeof(tesmio), _TRUNCATE, "%s\\tesmio", media);
    _snprintf_s(proc, sizeof(proc), _TRUNCATE, "%s\\procedural", tesmio);
    unsigned __int64 folderHash = folder ? SmlHashBytes(state->seed, folder, strlen(folder)) : state->seed;
    char worldId[32]; _snprintf_s(worldId, sizeof(worldId), _TRUNCATE, "%016llx", (unsigned long long)folderHash);
    _snprintf_s(world, sizeof(world), _TRUNCATE, "%s\\%s", proc, worldId);
    if (!EnsureDir(media) || !EnsureDir(tesmio) || !EnsureDir(proc) || !EnsureDir(world))
        { free(data); return NULL; }

    char file[MAX_PATH], temp[MAX_PATH];
    _snprintf_s(file, sizeof(file), _TRUNCATE, "%s\\resourcemap%d.dds", world, DEP_MAP_EXTRA + extraIndex + 1);
    _snprintf_s(temp, sizeof(temp), _TRUNCATE, "%s.tmp", file);
    HANDLE h = CreateFileA(temp, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    DWORD wrote = 0; bool ok = h != INVALID_HANDLE_VALUE && WriteFile(h, data, DDS_BYTES, &wrote, NULL) && wrote == DDS_BYTES;
    if (h != INVALID_HANDLE_VALUE) CloseHandle(h);
    free(data);
    if (!ok || !MoveFileExA(temp, file, MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
        { DeleteFileA(temp); return NULL; }

    _snprintf_s(rel, sizeof(rel), _TRUNCATE, "tesmio/procedural/%s/resourcemap%d.dds",
                worldId, DEP_MAP_EXTRA + extraIndex + 1);
    return rel;
}

static void SanitizedName(const char* src, char* out, size_t n)
{
    size_t j = 0;
...
static bool SaveWorldManifest(const char* folder, const WorldDepositState* state)
{
    char directory[MAX_PATH], path[MAX_PATH], temp[MAX_PATH];
    if (!ResolveWorldDirectory(folder, directory, sizeof(directory), true))
    {
        Logf("maps     manifest path rejected for \"%s\" (Win32 %lu)",
             folder ? folder : "(null)", GetLastError());
        return false;
    }
    _snprintf_s(path, sizeof(path), _TRUNCATE, "%s\\%s", directory, DEPOSIT_MANIFEST);
    _snprintf_s(temp, sizeof(temp), _TRUNCATE, "%s.tmp", path);
    FILE* f = NULL;
    if (fopen_s(&f, temp, "wb") || !f)
    {
        Logf("maps     cannot create manifest temp %s (Win32 %lu)", temp, GetLastError());
        return false;
    }
    bool ok = fprintf(f, "[world]\r\nformat = %d\r\nseed = %016llx\r\n\r\n[deposits]\r\n",
                      MANIFEST_FORMAT, (unsigned long long)state->seed) > 0;
    for (int i = 0; ok && i < g_depCount; i++)
    {
        const DepositDef* d = &g_dep[i];
        if (d->map < DEP_MAP_EXTRA || d->type < 10 || d->type > 127 ||
            !state->initialized[d->type] || state->map[d->type] != d->map ||
            state->component[d->type] != d->component)
            continue;
        char name[256]; SanitizedName(d->name, name, sizeof(name));
        ok = fprintf(f, "%d = %s|%d|%d\r\n", d->type, name, d->map, d->component) > 0;
    }
    if (fclose(f) != 0) ok = false;
    if (!ok || !MoveFileExA(temp, path, MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
    {
        DWORD error = GetLastError();
        Logf("maps     failed replacing manifest %s (Win32 %lu)", path, error);
        DeleteFileA(temp); return false;
    }
    Logf("maps     manifest persisted atomically: %s", path);
    return true;
}
...
static void LoadExtraMaps(void* mp, const char* folder)
{
    ...
    LoadWorldManifest(folder, &g_worldState);
    ...
    for (int k = 0; k < g_extraCount; k++)
    {
        ...
        bool ownExists = o_FileExists ? o_FileExists(own, false, true) : false;
        if (!o_FileExists)
        {
            char physical[MAX_PATH], name[40];
            _snprintf_s(name, sizeof(name), _TRUNCATE, "resourcemap%d.dds",
                        DEP_MAP_EXTRA + k + 1);
            ownExists = ResolveWorldFile(folder, name, physical, sizeof(physical));
        }

        bool pending[128], invalidExisting = false;
        const char* src = own;
        const char* generated = BuildGeneratedMap(folder, &g_worldState, k, ownExists,
                                                   pending, &invalidExisting);
        if (invalidExisting)
        {
            g_extra[k].tex = NULL;
        ...
        if (generated && generated[0]) src = generated;
        else if (!ownExists)
        {
            Logf("maps     resourcemap%d: generation failed - using unsaved blank",
                 DEP_MAP_EXTRA + k + 1);
            src = BlankMapPath();
            memset(pending, 0, sizeof(pending));
            if (!src) { g_extra[k].tex = NULL; continue; }
        }
        if (o_FileExists && !o_FileExists(src, false, true))
        {
            Logf("maps     resourcemap%d: fallback %s is not visible through the VFS",
                 DEP_MAP_EXTRA + k + 1, src);
            g_extra[k].tex = NULL;
            if (g_extra[k].caveSlot) *g_extra[k].caveSlot = NULL;
            continue;
        }
        ...
        void* loadFn = TexVCall(tex, TEX_LOAD2D);
        void* initFn = TexVCall(tex, TEX_INIT_TEMP);
        if (!loadFn || !initFn)
        {
            Logf("maps     resourcemap%d: texture has no load/init interface",
                 DEP_MAP_EXTRA + k + 1);
            g_extra[k].tex = NULL;
            if (g_extra[k].caveSlot) *g_extra[k].caveSlot = NULL;
            continue;
        ...
        ((t_Load)loadFn)(tex, src, 0, 0, 0, 0);
        ((t_Init)initFn)(tex);

        g_extra[k].tex = tex;
        if (g_extra[k].caveSlot) *g_extra[k].caveSlot = tex;
        g_worldState.saveEligible[k] = ownExists || (generated && generated[0]);

        for (int i = 0; i < g_depCount; i++)
        {
            DepositDef* d = &g_dep[i];
            if (d->type < 10 || d->type > 127 || !pending[d->type]) continue;
            g_worldState.initialized[d->type] = true;
            g_worldState.map[d->type] = d->map;
            g_worldState.component[d->type] = d->component;
        }

        Logf("maps     resourcemap%d = %p (%s)", DEP_MAP_EXTRA + k + 1, tex,
             src == own ? own : (generated && generated[0]) ? "generated" : "blank");
    }
    g_worldState.manifestValid = true;
}
...
static void h_WorldSaveMaps(void* self, const char* folder)
{
    ...
    __try
    {
        bool persisted[MAX_EXTRA_MAPS] = {};
        typedef void (*t_Save)(void*, const char*);
        for (int k = 0; k < g_extraCount; k++)
        {
            if (!g_extra[k].tex || !g_worldState.saveEligible[k]) continue;
        ...
        WorldDepositState saved = g_worldState;
        for (int i = 0; i < g_depCount; i++)
        {
            DepositDef* d = &g_dep[i];
            int k = d->map - DEP_MAP_EXTRA;
            if (k >= 0 && k < MAX_EXTRA_MAPS && !persisted[k] && d->type >= 10 && d->type <= 127)
                saved.initialized[d->type] = false;
        }
        if (!SaveWorldManifest(folder, &saved))
            Logf("maps     failed writing %s/%s", folder, DEPOSIT_MANIFEST);
        else
        {
            Logf("maps     saved deposit manifest to %s/%s (seed %016llx)",
                 folder, DEPOSIT_MANIFEST, (unsigned long long)saved.seed);
            g_worldState = saved;
        }
    }
...
{
    g_smlMapsReady = false;
    if (g_extraCount <= 0) { g_smlMapsReady = true; return; }
    ...
    bool saveHookReady = PatchWorldSaveCall((void*)h_WorldSaveMaps);
    if (!saveHookReady)
...
static void EmitDispatchCase(Emit& e, const DepositDef* d, BYTE* mapSlot)
{
    ...
    if (mapSlot)
    {
        ...
        e.b(0x4D); e.b(0x85); e.b(0xC9);                                   // test r9,r9
        noTexture = e.je32();                                               // absent map -> zero richness
    }
...
static bool PatchDepositType()
{
    ...
    {
        Logf("patch  site bytes differ from build v1.1.1.9 - refusing to patch");
        return false;
    }
    ...
    {
        Logf("patch  resource-map bracket bytes differ from build v1.1.1.9 - "
    ...
    {
        Logf("patch  terrain-mask bracket bytes differ from build v1.1.1.9 - "
...
extern "C" int SmlDepositsDeclaredCount(void) { return g_depCount; }
extern "C" int SmlDepositsPatchReady(void) { return g_smlPatchReady ? 1 : 0; }
extern "C" int SmlDepositsMapsReady(void) { return g_smlMapsReady ? 1 : 0; }
...
extern "C" int TsmPluginInit(const TsmHost* host, TsmPluginInfo* info)
{
    ...
    if (g_depositPatch) g_smlPatchReady = PatchDepositType();
    else g_smlPatchReady = g_depCount == 0;
